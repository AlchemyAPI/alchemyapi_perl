package AlchemyAPI_RelationParams;
...
sub new() {
	...
	my $self = {
		...
		_coreference => undef,
		_entities => undef,
		_sentimentExcludeEntities => undef,
		_requireEntities => undef,
...
sub SetEntities {
	...
	if( 0 == $entities || 1 == $entities ) {
		$self->{_entities} = $entities;
	}
	else {
		throw Error::Simple( "Error:  Cannot set Entities to ".$entities);
	}
}
...
sub SetSentimentExcludeEntities {
	my($self, $sentimentExcludeEntities) = @_;
	
	if( 0 == $sentimentExcludeEntities || 1 == $sentimentExcludeEntities ) {
		$self->{_sentimentExcludeEntities} = $sentimentExcludeEntities;
	}
	else {
		throw Error::Simple( "Error:  Cannot set SentimentExcludeEntities to ".$sentimentExcludeEntities);
	}
}

sub GetSentimentExcludeEntities {
	my($self) = @_;
	return $self->{_sentimentExcludeEntities};
}

sub SetRequireEntities {
	my($self, $requireEntities) = @_;
	
	if( 0 == $requireEntities || 1 == $requireEntities ) {
		$self->{_requireEntities} = $requireEntities;
	}
	else {
		throw Error::Simple( "Error:  Cannot set RequireEntities to ".$requireEntities);
	}
}

sub GetRequireEntities {
	my($self) = @_;
...
sub GetParameterString {
	...
	if( defined $self->{_entities} ) {
		$retString .= "&entities=".$self->{_entities};
	}
	if( defined $self->{_sentimentExcludeEntities} ) {
		$retString .= "&sentimentExcludeEntities=".$self->{_sentimentExcludeEntities};
	}
	if( defined $self->{_requireEntities} ) {
		$retString .= "&requireEntities=".$self->{_requireEntities};
	}
